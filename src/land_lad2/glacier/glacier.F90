...
character(len=*), parameter :: &
       module_name = 'glacier',&
       version     = '$Id: glacier.F90,v 1.1.2.12 2012/06/19 18:34:54 pjp Exp $',&
...
subroutine read_glac_namelist()
  ...
  do 
     read (nml_unit, nml=glac_nml, iostat=io, end=10)
     if (check_nml_error (io, 'glac_nml')==0) exit ! from loop
  enddo
10 close (nml_unit)
  write (logunit, nml=glac_nml)
...
subroutine glac_init ( id_lon, id_lat )
  ...
  if (restart_exists) then
     ...
     call error_mesg('glac_init', 'cold-starting glacier', NOTE)
...
subroutine glac_diag_init ( id_lon, id_lat, zfull, zhalf )
  ...
  id_lwc  = register_tiled_diag_field ( trim(module_name), 'glac_liq', 'bulk density of liquid water', 'kg/m3')
  id_swc  = register_tiled_diag_field ( trim(module_name), 'glac_ice', 'bulk density of solid water',  'kg/m3')
  id_temp = register_tiled_diag_field ( trim(module_name), 'glac_T',   'temperature',                  'degK')
if (.not.lm2) then
  id_ie  = register_tiled_diag_field ( trim(module_name), 'glac_rie', 'inf exc runf', 'kg/(m2 s)')
  id_sn  = register_tiled_diag_field ( trim(module_name), 'glac_rsn', 'satn runf',    'kg/(m2 s)')
  id_bf  = register_tiled_diag_field ( trim(module_name), 'glac_rbf', 'baseflow',     'kg/(m2 s)')
  id_hie = register_tiled_diag_field ( trim(module_name), 'glac_hie', 'heat ie runf', 'W/m2')
  id_hsn = register_tiled_diag_field ( trim(module_name), 'glac_hsn', 'heat sn runf', 'W/m2')
  id_hbf = register_tiled_diag_field ( trim(module_name), 'glac_hbf', 'heat bf runf', 'W/m2')
endif
