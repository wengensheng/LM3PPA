...
use constants_mod,      only: tfreeze, hlv, hlf, dens_h2o, PI, grav, vonkarm, rdgas

use land_constants_mod, only : NBANDS
use lake_tile_mod, only : &
     lake_tile_type, lake_pars_type, lake_prog_type, read_lake_data_namelist, &
     lake_data_radiation, lake_data_thermodynamics, &
...
character(len=*), parameter, private   :: &
    module_name = 'lake',&
    version     = '$Id: lake.F90,v 1.1.2.12 2012/06/19 18:34:54 pjp Exp $',&
...
real    :: K_z_large            = 1.
real    :: connected_to_next    = 0.
real    :: whole_lake_area      = 0.
real    :: lake_depth_sill      = 0.
...
namelist /lake_nml/ init_temp, init_w,       &
                    ...
                    connected_to_next, whole_lake_area, lake_depth_sill
!---- end of namelist --------------------------------------------------------
...
subroutine read_lake_namelist()
  ...
  do 
     read (nml_unit, nml=lake_nml, iostat=io, end=10)
     if (check_nml_error (io, 'lake_nml')==0) exit ! from loop
  enddo
10  close (nml_unit)
  write(logunit, nml=lake_nml)
...
subroutine lake_diag_init ( id_lon, id_lat )
  ...
  id_sillw = register_tiled_static_field ( trim(module_name), 'lake_width', 'lake width at outflow', 'm')
  id_silld = register_tiled_static_field ( trim(module_name), 'lake_depth', 'lake depth below sill', 'm')

  ! define dynamic diagnostic fields
  id_dz   = register_tiled_diag_field ( trim(module_name), 'lake_dz',   'nominal layer thickness', 'm')
  id_wl   = register_tiled_diag_field ( trim(module_name), 'lake_wl',   'liquid water mass', 'kg/m2')
  id_ws   = register_tiled_diag_field ( trim(module_name), 'lake_ws',   'solid water mass', 'kg/m2')
  id_lwc  = register_tiled_diag_field ( trim(module_name), 'lake_liq',  'bulk density of liquid water', 'kg/m3')
  id_swc  = register_tiled_diag_field ( trim(module_name), 'lake_ice',  'bulk density of solid water', 'kg/m3')
  id_temp = register_tiled_diag_field ( trim(module_name), 'lake_T',    'temperature', 'degK')
  id_K_z  = register_tiled_diag_field ( trim(module_name), 'lake_K_z',  'vertical diffusivity', 'm2/s')
  id_evap = register_tiled_diag_field ( trim(module_name), 'lake_evap', 'lake evap', 'kg/(m2 s)')
