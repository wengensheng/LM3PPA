...
use time_manager_mod, only : time_type, get_date, time_type_to_real, operator(+), &
                             print_date

use constants_mod, only : radius, hlf, hlv, hls, tfreeze, pi, rdgas, rvgas, cp_air, stefan
...
use snow_mod, only : read_snow_namelist, snow_init, snow_end, snow_get_sfc_temp, &
     snow_radiation, snow_get_depth_area, snow_step_1, snow_step_2, save_snow_restart
...
use canopy_air_mod, only : read_cana_namelist, cana_init, cana_end, cana_state,&
     cana_step_1, cana_step_2, cana_roughness, save_cana_restart
use river_mod, only : river_init, river_end, update_river, save_river_restart
...
character(len=*), parameter :: &
     module_name = 'land', &
     version     = '$Id: land_model.F90,v 1.1.2.1.2.17 2012/06/19 18:34:54 pjp Exp $', &
...
character(16) :: nearest_point_search = 'dummy' ! dummy placeholder
...
!
  ...
  id_cellarea, id_landarea, id_no_riv,                                     &
...
! so critical. 
  ...
  do 
     ...
     if (check_nml_error (io, 'land_model_nml')==0) exit ! from loop
  enddo
10 close (unit)

  write (logunit, nml=land_model_nml)
  ...
  call land_diag_init(lnd%coord_glonb, lnd%coord_glatb, lnd%coord_glon, lnd%coord_glat, time, id_lon, id_lat, id_band)
  ...
  write(id_cellarea, diag_format) year, month, day, hour, minute, second, lnd%cellarea(lnd%is:lnd%ie,lnd%js:lnd%je)
  write(id_landarea, diag_format) year, month, day, hour, minute, second, lnd%area(lnd%is:lnd%ie,lnd%js:lnd%je)
  write(id_geolon_t, diag_format) year, month, day, hour, minute, second, lnd%lon(lnd%is:lnd%ie,lnd%js:lnd%je)
  write(id_geolat_t, diag_format) year, month, day, hour, minute, second, lnd%lat(lnd%is:lnd%ie,lnd%js:lnd%je)
  ...
  call topo_rough_init( lnd%time, lnd%lonb, lnd%latb, id_lon, id_lat)
  ...
  write(id_no_riv, diag_format) year, month, day, hour, minute, second, no_riv
  ...
  ! flags
  if (canopy_air_mass_for_tracers==0.and.lnd%ico2==0) then
     call error_mesg('land_model_init', &
...
!  call error_mesg('land_model_end','writing NetCDF restart',NOTE)
  ...
  ! if the number of tiles is zero, in case they are doing 
...
subroutine land_cover_cold_start(lnd)
  ...
  lake=>lake_cover_cold_start(land_mask,lnd%lonb,lnd%latb)
  ...
  ! reconcile vegetation fractions with the land mask
...
subroutine update_land_model_fast ( cplr2land, land2cplr )
  ...
  allocate(phot_co2_data(is:ie,js:je)) ; phot_co2_data = 0.0
...
!     __DEBUG4__(is,js,i-is+lnd%is,j-js+lnd%js)
     ...
     do while (ce/=te)
        ...
        ! convenience: the 2d and 3d vars are generally not 
...
!  write(id_dis_liq,  diag_format) year, month, day, hour, minute, second, discharge_l
!  write(id_dis_ice,  diag_format) year, month, day, hour, minute, second, discharge_c(:,:,1)
!  write(id_dis_heat, diag_format) year, month, day, hour, minute, second, discharge_c(:,:,2)
!  write(id_dis_sink, diag_format) year, month, day, hour, minute, second, discharge_sink
...
subroutine update_land_model_fast_0d ( tile, ix,iy,itile, N, land2cplr, &
  ...
  if (associated(tile%glac)) then
     ...
     call error_mesg('update_land_model_fast','none of the surface tiles exist' ,FATAL)
  endif
...
! ALSO, DIAGNOSTICS IN COMPONENT MODULES SHOULD _FOLLOW_ THIS ADJUSTMENT******
  ...
  if (associated(tile%vegn)) then
     ...
     call vegn_step_3 (tile%vegn, tile%soil, tile%cana%prog%T, precip_l+precip_s, &
          vegn_fco2, tile%diag)
     ! put runoff into vegn tile
     tile%vegn%Wrunoff= subs_lrunf  + snow_lrunf
  endif
...
!  call send_tile_data(id_hfprecs, vegn_hfprec,                        tile%diag)
  ...
  evap_only = sum(vegn_levap(:)*f(:)) +      &
              snow_levap + subs_levap +      &
              sum(vegn_fevap(:)*f(:)) +      &
              snow_fevap+subs_fevap
  call send_tile_data(id_evap2,   evap_only,                          tile%diag)
...
!  call send_tile_data(id_con_g_h, con_g_h,                            tile%diag)
  ...
  call send_tile_data(id_transp2, sum(f(:)*vegn_uptk),                tile%diag)
...
subroutine update_land_bc_fast (tile, i,j,k, land2cplr, is_init)
  ...
  if (associated(tile%glac)) then
     ...
     call error_mesg('update_land_bc_fast','none of the surface tiles exist', FATAL)
  endif
...
subroutine land_diag_init(clonb, clatb, clon, clat, time, id_lon, id_lat, id_band)
  real, intent(in) :: &
  ...
  integer, intent(out) :: id_lon, id_lat, id_band   ! IDs of respective diag. manager axes
 ...
 id_geolon_t = get_unit()
 open(unit=id_geolon_t, file='DIAGNOSTICS/'//trim(module_name)//'_geolon_t', form='formatted', action='write', position='rewind')
 write(id_geolon_t,'(a)') 'units = degrees_E'
 id_geolat_t = get_unit()
 open(unit=id_geolat_t, file='DIAGNOSTICS/'//trim(module_name)//'_geolat_t', form='formatted', action='write', position='rewind')
 write(id_geolat_t,'(a)') 'units = degrees_N'

  ! register static diagnostic fields

 id_cellarea = get_unit()
 open(unit=id_cellarea, file='DIAGNOSTICS/'//trim(module_name)//'_cellarea', form='formatted', action='write', position='rewind')
 write(id_cellarea,'(a)') 'units = m2'
 id_landarea = get_unit()
 open(unit=id_landarea, file='DIAGNOSTICS/'//trim(module_name)//'_landarea', form='formatted', action='write', position='rewind')
 write(id_landarea,'(a)') 'units = m2'
 id_no_riv = get_unit()
 open(unit=id_no_riv, file='DIAGNOSTICS/'//trim(module_name)//'_no_riv', form='formatted', action='write', position='rewind')
 write(id_no_riv,'(a)') 'units = unitless'

 ! register regular (dynamic) diagnostic fields

 id_VWS = register_tiled_diag_field ( trim(module_name), 'VWS',  'vapor storage on land', 'kg/m2')
 id_VWSc= register_tiled_diag_field ( trim(module_name), 'VWSc', 'vapor mass in canopy air', 'kg/m2')
 id_LWS = register_tiled_diag_field ( trim(module_name), 'LWS',  'liquid storage on land', 'kg/m2')
 id_LWSv= register_tiled_diag_field ( trim(module_name), 'LWSv', 'liquid interception storage', 'kg/m2')
 id_LWSs= register_tiled_diag_field ( trim(module_name), 'LWSs', 'liquid storage in snowpack', 'kg/m2')
 id_LWSg= register_tiled_diag_field ( trim(module_name), 'LWSg', 'liquid ground storage', 'kg/m2')
 id_FWS = register_tiled_diag_field ( trim(module_name), 'FWS',  'frozen storage on land', 'kg/m2')
 id_FWSv= register_tiled_diag_field ( trim(module_name), 'FWSv', 'frozen interception storage', 'kg/m2')
 id_FWSs= register_tiled_diag_field ( trim(module_name), 'FWSs', 'frozen storage in snowpack', 'kg/m2')
 id_FWSg= register_tiled_diag_field ( trim(module_name), 'FWSg', 'frozen ground storage', 'kg/m2')
 id_HS  = register_tiled_diag_field ( trim(module_name), 'HS',   'land heat storage', 'J/m2')
 id_HSv = register_tiled_diag_field ( trim(module_name), 'HSv',  'interception heat storage', 'J/m2')
 id_HSs = register_tiled_diag_field ( trim(module_name), 'HSs',  'heat storage in snowpack', 'J/m2')
 id_HSg = register_tiled_diag_field ( trim(module_name), 'HSg',  'ground heat storage', 'J/m2')
 id_HSc = register_tiled_diag_field ( trim(module_name), 'HSc',  'canopy-air heat storage', 'J/m2')

 id_dis_liq = get_unit()
 open(unit=id_dis_liq, file='DIAGNOSTICS/'//trim(module_name)//'_dis_liq', form='formatted', action='write', position='rewind')
 write(id_dis_liq,'(a)') 'units = kg/(m2 s)'
 id_dis_ice = get_unit()
 open(unit=id_dis_ice, file='DIAGNOSTICS/'//trim(module_name)//'_dis_ice', form='formatted', action='write', position='rewind')
 write(id_dis_ice,'(a)') 'units = kg/(m2 s)'
 id_dis_heat = get_unit()
 open(unit=id_dis_heat, file='DIAGNOSTICS/'//trim(module_name)//'_dis_heat', form='formatted', action='write', position='rewind')
 write(id_dis_heat,'(a)') 'units = W/m2'
 id_dis_sink = get_unit()
 open(unit=id_dis_sink, file='DIAGNOSTICS/'//trim(module_name)//'_dis_sink', form='formatted', action='write', position='rewind')
 write(id_dis_sink,'(a)') 'units = kg/(m2 s)'

 id_precip  = register_tiled_diag_field ( trim(module_name), 'precip', 'precipitation rate', 'kg/(m2 s)')
 id_hprec   = register_tiled_diag_field ( trim(module_name), 'hprec',  'sensible heat of precipitation', 'W/m2')
 id_lprec   = register_tiled_diag_field ( trim(module_name), 'lprec_l','rainfall to land', 'kg/(m2 s)')
 id_lprecv  = register_tiled_diag_field ( trim(module_name), 'lprecv', 'net rainfall to vegetation', 'kg/(m2 s)')
 id_lprecs  = register_tiled_diag_field ( trim(module_name), 'lprecs', 'rainfall to snow, minus drainage', 'kg/(m2 s)')
 id_lprecg  = register_tiled_diag_field ( trim(module_name), 'lprecg', 'effective rainfall to ground sfc', 'kg/(m2 s)')
 id_hlprec  = register_tiled_diag_field ( trim(module_name), 'hlprec', 'total liq precipitation heat', 'W/m2')
 id_hlprecv = register_tiled_diag_field ( trim(module_name), 'hlprecv','net liq heat to vegetation', 'W/m2')
 id_hlprecs = register_tiled_diag_field ( trim(module_name), 'hlprecs','net liq heat to snow', 'W/m2')
 id_hlprecg = register_tiled_diag_field ( trim(module_name), 'hlprecg','net liq heat to ground sfc', 'W/m2')
 id_fprec   = register_tiled_diag_field ( trim(module_name), 'fprec_l','snowfall to land', 'kg/(m2 s)')
 id_fprecv  = register_tiled_diag_field ( trim(module_name), 'fprecv', 'net snowfall to vegetation', 'kg/(m2 s)')
 id_fprecs  = register_tiled_diag_field ( trim(module_name), 'fprecs', 'effective snowfall to snowpack', 'kg/(m2 s)')
 id_hfprec  = register_tiled_diag_field ( trim(module_name), 'hfprec', 'sens heat of snowfall', 'W/m2')
 id_hfprecv = register_tiled_diag_field ( trim(module_name), 'hfprecv','net sol heat to vegetation', 'W/m2')
 id_hfprecs = register_tiled_diag_field ( trim(module_name), 'hfprecs','net sol heat to snow', 'W/m2')
 id_evap    = register_tiled_diag_field ( trim(module_name), 'evap',   'vapor flux up from land', 'kg/(m2 s)')
 id_evap2   = register_tiled_diag_field ( trim(module_name), 'evap2',  'land surface evaporation', 'kg/(m2 s)')
 id_hevap   = register_tiled_diag_field ( trim(module_name), 'hevap',  'sensible heat of evap', 'W/m2')
 id_levap   = register_tiled_diag_field ( trim(module_name), 'levap',  'vapor flux from all liq (inc Tr)', 'kg/(m2 s)')
 id_levapv  = register_tiled_diag_field ( trim(module_name), 'levapv', 'vapor flux leaving intercepted liquid', 'kg/(m2 s)')
 id_levaps  = register_tiled_diag_field ( trim(module_name), 'levaps', 'vapor flux leaving snow liquid', 'kg/(m2 s)')
 id_levapg  = register_tiled_diag_field ( trim(module_name), 'levapg', 'vapor flux from ground liquid', 'kg/(m2 s)')
 id_hlevap  = register_tiled_diag_field ( trim(module_name), 'hlevap', 'vapor flux heat from liq source', 'W/m2')
 id_hlevapv = register_tiled_diag_field ( trim(module_name), 'hlevapv','vapor heat from liq interc', 'W/m2')
 id_hlevaps = register_tiled_diag_field ( trim(module_name), 'hlevaps','vapor heat from snow liq', 'W/m2')
 id_hlevapg = register_tiled_diag_field ( trim(module_name), 'hlevapg','vapor heat from ground liq', 'W/m2')
 id_fevap   = register_tiled_diag_field ( trim(module_name), 'fevap',  'vapor flux from all ice', 'kg/(m2 s)')
 id_fevapv  = register_tiled_diag_field ( trim(module_name), 'fevapv', 'vapor flux leaving vegn ice', 'kg/(m2 s)')
 id_fevaps  = register_tiled_diag_field ( trim(module_name), 'fevaps', 'vapor flux leaving snow ice', 'kg/(m2 s)')
 id_fevapg  = register_tiled_diag_field ( trim(module_name), 'fevapg', 'vapor flux leaving ground ice', 'kg/(m2 s)')
 id_hfevap  = register_tiled_diag_field ( trim(module_name), 'hfevap', 'vapor flux heat from solid source', 'W/m2')
 id_hfevapv = register_tiled_diag_field ( trim(module_name), 'hfevapv','vapor heat from sol interc', 'W/m2')
 id_hfevaps = register_tiled_diag_field ( trim(module_name), 'hfevaps','vapor heat from snow sol', 'W/m2')
 id_hfevapg = register_tiled_diag_field ( trim(module_name), 'hfevapg','vapor heat from ground sol', 'W/m2)')
 id_runf    = register_tiled_diag_field ( trim(module_name), 'runf',   'total runoff', 'kg/(m2 s)')
 id_hrunf   = register_tiled_diag_field ( trim(module_name), 'hrunf',  'sensible heat of total runoff', 'W/m2')
 id_lrunf   = register_tiled_diag_field ( trim(module_name), 'lrunf',  'total rate of liq runoff', 'kg/(m2 s)')
 id_lrunfs  = register_tiled_diag_field ( trim(module_name), 'lrunfs', 'rate of liq runoff via calving', 'kg/(m2 s)')
 id_lrunfg  = register_tiled_diag_field ( trim(module_name), 'lrunfg', 'rate of liq runoff, ground', 'kg/(m2 s)')
 id_hlrunf  = register_tiled_diag_field ( trim(module_name), 'hlrunf', 'heat of liq runoff', 'W/m2')
 id_hlrunfs = register_tiled_diag_field ( trim(module_name), 'hlrunfs','heat of liq runoff from snow pack', 'W/m2')
 id_hlrunfg = register_tiled_diag_field ( trim(module_name), 'hlrunfg','heat of liq surface runoff', 'W/m2')
 id_frunf   = register_tiled_diag_field ( trim(module_name), 'frunf',  'total rate of solid runoff', 'kg/(m2 s)')
 id_frunfs  = register_tiled_diag_field ( trim(module_name), 'frunfs', 'rate of solid calving', 'kg/(m2 s)')
 id_hfrunf  = register_tiled_diag_field ( trim(module_name), 'hfrunf', 'heat of total ice runoff', 'W/m2')
 id_hfrunfs = register_tiled_diag_field ( trim(module_name), 'hfrunfs','heat of sol snow runoff', 'W/m2')
 id_melt    = register_tiled_diag_field ( trim(module_name), 'melt',   'total rate of melt', 'kg/(m2 s)')
 id_meltv   = register_tiled_diag_field ( trim(module_name), 'meltv',  'rate of melt, interception', 'kg/(m2 s)')
 id_melts   = register_tiled_diag_field ( trim(module_name), 'melts',  'rate of snow melt', 'kg/(m2 s)')
 id_meltg   = register_tiled_diag_field ( trim(module_name), 'meltg',  'rate of substrate thaw', 'kg/(m2 s)')
 id_fsw     = register_tiled_diag_field ( trim(module_name), 'fsw',    'net sw rad to land', 'W/m2')
 id_fswv    = register_tiled_diag_field ( trim(module_name), 'fswv',   'net sw rad to vegetation', 'W/m2')
 id_fsws    = register_tiled_diag_field ( trim(module_name), 'fsws',   'net sw rad to snow', 'W/m2')
 id_fswg    = register_tiled_diag_field ( trim(module_name), 'fswg',   'net sw rad to ground', 'W/m2')
 id_flw     = register_tiled_diag_field ( trim(module_name), 'flw',    'net lw rad to land', 'W/m2')
 id_flwv    = register_tiled_diag_field ( trim(module_name), 'flwv',   'net lw rad to vegetation', 'W/m2')
 id_flws    = register_tiled_diag_field ( trim(module_name), 'flws',   'net lw rad to snow', 'W/m2')
 id_flwg    = register_tiled_diag_field ( trim(module_name), 'flwg',   'net lw rad to ground', 'W/m2')
 id_sens    = register_tiled_diag_field ( trim(module_name), 'sens',   'sens heat flux from land', 'W/m2')
 id_sensv   = register_tiled_diag_field ( trim(module_name), 'sensv',  'sens heat flux from vegn', 'W/m2')
 id_senss   = register_tiled_diag_field ( trim(module_name), 'senss',  'sens heat flux from snow', 'W/m2')
 id_sensg   = register_tiled_diag_field ( trim(module_name), 'sensg',  'sens heat flux from ground', 'W/m2')
 id_e_res_1 = register_tiled_diag_field ( trim(module_name),'e_res_1', 'canopy air energy residual due to nonlinearities','W/m2')
 id_e_res_2 = register_tiled_diag_field ( trim(module_name), 'e_res_2', 'canopy energy residual due to nonlinearities', 'W/m2')
 id_frac    = register_tiled_diag_field ( trim(module_name), 'frac',   'fraction of land area', 'unitless')
 id_area    = register_tiled_diag_field ( trim(module_name), 'area',   'area in the grid cell', 'm2')
 id_ntiles  = register_tiled_diag_field ( trim(module_name), 'ntiles', 'number of tiles', 'unitless')
 id_z0m     = register_tiled_diag_field ( trim(module_name), 'z0m',    'momentum roughness of land', 'm')
 id_z0s     = register_tiled_diag_field ( trim(module_name), 'z0s',    'scalar roughness of land', 'm')
 id_con_g_h = register_tiled_diag_field ( trim(module_name), 'con_g_h',  &
              'conductance for sensible heat between ground surface and canopy air', 'm/s')
 id_transp  = register_tiled_diag_field ( trim(module_name), 'transp',   'transpiration; = uptake by roots', 'kg/(m2 s)')
 id_transp2 = register_tiled_diag_field ( trim(module_name), 'transptot','total transpiration; = uptake by roots', 'kg/(m2 s)')
 id_wroff   = register_tiled_diag_field ( trim(module_name), 'wroff',    'rate of liquid runoff to rivers', 'kg/(m2 s)')
 id_sroff   = register_tiled_diag_field ( trim(module_name), 'sroff',    'rate of solid runoff to rivers', 'kg/(m2 s)')
 id_htransp = register_tiled_diag_field ( trim(module_name), 'htransp',  'heat of transpired vapior', 'W/m2')
 id_huptake = register_tiled_diag_field ( trim(module_name), 'huptk',    'heat of soil water uptake', 'W/m2')
 id_hroff   = register_tiled_diag_field ( trim(module_name), 'hroff',    'sensible heat of runoff', 'W/m2')
 id_gsnow   = register_tiled_diag_field ( trim(module_name), 'gsnow',    'sens heat into ground from snow', 'W/m2')
 id_gequil  = register_tiled_diag_field ( trim(module_name), 'gequil',   'snow-subs equilibration flux', 'W/m2')
 id_grnd_flux =register_tiled_diag_field( trim(module_name), 'grnd_flux','sensible heat into ground from surface', 'W/m2')
 id_levapg_max =register_tiled_diag_field(trim(module_name),'Eg_max',  &
              'soil_water limit on vapor flux from ground liquid','kg/(m2 s)')
 id_water = register_tiled_diag_field(trim(module_name), 'water',   'column-integrated soil water', 'kg/m2')
 id_snow = register_tiled_diag_field(trim(module_name), 'snow',     'column-integrated snow water', 'kg/m2')
 id_Trad    = register_tiled_diag_field(trim(module_name), 'Trad',  'radiative sfc temperature', 'degK')
 id_Tca     = register_tiled_diag_field(trim(module_name), 'Tca',   'canopy-air temperature', 'degK')
 id_qca     = register_tiled_diag_field(trim(module_name), 'qca',   'canopy-air specific humidity', 'kg/kg')
 id_qco2    = register_tiled_diag_field(trim(module_name), 'qco2',  'canopy-air CO2 moist mass mixing ratio', 'kg/kg')
 id_qco2_dvmr = register_tiled_diag_field(trim(module_name), 'qco2_dvmr',   &
               'canopy-air CO2 dry volumetric mixing ratio', 'mol CO2/mol air')
 id_fco2    = register_tiled_diag_field(trim(module_name), 'fco2',     'flux of CO2 to canopy air', 'kg C/(m2 s)')
 id_swdn_dir = register_tiled_diag_field(trim(module_name), 'swdn_dir', &
               'downward direct short-wave radiation flux to the land surface', 'W/m2')
 id_swdn_dif = register_tiled_diag_field(trim(module_name), 'swdn_dif', &
               'downward diffuse short-wave radiation flux to the land surface', 'W/m2')
 id_swup_dir = register_tiled_diag_field(trim(module_name), 'swup_dir', &
               'direct short-wave radiation flux reflected by the land surface', 'W/m2')
 id_swup_dif = register_tiled_diag_field(trim(module_name), 'swup_dif', &
               'diffuse short-wave radiation flux reflected by the land surface', 'W/m2')
 id_lwdn = register_tiled_diag_field(trim(module_name), 'lwdn', 'downward long-wave radiation flux to the land surface', 'W/m2')
 id_vegn_cover = register_tiled_diag_field(trim(module_name), 'vegn_cover',    'fraction covered by vegetation')
 id_cosz = register_tiled_diag_field(trim(module_name), 'coszen',    'cosine of zenith angle')
 id_albedo_dir = register_tiled_diag_field(trim(module_name), 'albedo_dir', 'land surface albedo for direct light')
 id_albedo_dif = register_tiled_diag_field(trim(module_name), 'albedo_dif', 'land surface albedo for diffuse light')
 id_vegn_refl_dir = register_tiled_diag_field(trim(module_name), 'vegn_refl_dir',  &
               'black-background canopy reflectivity for direct light')
 id_vegn_refl_dif = register_tiled_diag_field(trim(module_name), 'vegn_refl_dif',  &
               'black-background canopy reflectivity for diffuse light')
 id_vegn_refl_lw = register_tiled_diag_field(trim(module_name), 'vegn_refl_lw',   'canopy reflectivity for thermal radiation')
 id_vegn_tran_dir = register_tiled_diag_field(trim(module_name), 'vegn_tran_dir',  &
               'part of direct light that passes through canopy unscattered')
 id_vegn_tran_dif = register_tiled_diag_field(trim(module_name), 'vegn_tran_dif',  &
               'black-background canopy transmittance for diffuse light')
 id_vegn_tran_lw = register_tiled_diag_field(trim(module_name), 'vegn_tran_lw',   'canopy transmittance for thermal radiation')
 id_vegn_sctr_dir = register_tiled_diag_field(trim(module_name), 'vegn_sctr_dir',  &
               'part of direct light scattered downward by canopy')
 id_subs_refl_dir = register_tiled_diag_field(trim(module_name), 'subs_refl_dir',  'substrate reflectivity for direct light')
 id_subs_refl_dif = register_tiled_diag_field(trim(module_name), 'subs_refl_dif',  'substrate reflectivity for diffuse light')
 id_grnd_T = register_tiled_diag_field( trim(module_name), 'Tgrnd',     'ground surface temperature', 'degK')
 id_subs_emis = register_tiled_diag_field(trim(module_name), 'subs_emis', 'substrate emissivity for long-wave radiation')

 id_water_cons = register_tiled_diag_field(trim(module_name), 'water_cons', 'water non-conservation in update_land_model_fast_0d', 'kg/(m2 s)')
 id_carbon_cons = register_tiled_diag_field(trim(module_name), 'carbon_cons', 'carbon non-conservation in update_land_model_fast_0d', 'kgC/(m2 s)')
...
! enumerate all the tiles, call accessor routine for each of them, and
