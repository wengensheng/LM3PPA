...
character(len=*), parameter, private   :: &
       ...
       version     = '$Id: snow.F90,v 1.1.2.7 2012/06/19 18:34:54 pjp Exp $' ,&
...
subroutine read_snow_namelist()
  ...
  do 
     read (nml_unit, nml=snow_nml, iostat=io, end=10)
     if (check_nml_error (io, 'snow_nml')==0) exit ! from loop
  enddo
10  close (nml_unit)
...
subroutine snow_init ( id_lon, id_lat )
  ...
  if (restart_exists) then
     call error_mesg('snow_init',&
          'reading NetCDF restart "'//trim(restart_file_name)//'"', NOTE)
