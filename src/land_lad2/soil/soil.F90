...
character(len=*), parameter, private   :: &
    module_name = 'soil',&
    version     = '$Id: soil.F90,v 1.1.2.13 2012/06/19 18:34:54 pjp Exp $',&
...
!real    :: hillslope_length = 1000.
!real    :: hillslope_zeta_bar = 0.5
!real    :: soil_e_depth = 3.0
!real    :: slope     = 0.17161244356631780E-02
!real    :: f_iso_vis = 0.49504825182177040E-01
!real    :: f_vol_vis = 0.83999998867511749E-01
!real    :: f_geo_vis = 0.23000000510364767E-02
!real    :: f_iso_nir = 0.22851798415092606E+00
!real    :: f_vol_nir = 0.98999999463558169E-01
!real    :: f_geo_nir = 0.18999999389052391E-01
real    :: tau       = 5.734E6 ! Not used. Model uses tau only when use_geohydrology=.false. This model is intended to run with use_geohydrology=.true.

real    :: hillslope_length   = Z'408F400000000000' 
real    :: hillslope_zeta_bar = Z'3FE0000000000000'
real    :: soil_e_depth = Z'4008000000000001'
real    :: slope     = Z'3FC5F765845A1C09'
real    :: f_iso_vis = Z'3FA958B24A4DA4C2'
real    :: f_vol_vis = Z'3FB5810620000000'
real    :: f_geo_vis = Z'3F62D7731FFFFFFF'
real    :: f_iso_nir = Z'3FCD4013CA3CEFFC'
real    :: f_vol_nir = Z'3FB958105FFFFFFE'
real    :: f_geo_nir = Z'3F9374BC60000000'
...
namelist /soil_nml/ lm2, use_E_min, use_E_max,           &
                    ...
                    hillslope_length, hillslope_zeta_bar, soil_e_depth, slope, &
                    f_iso_vis, f_vol_vis, f_geo_vis, &
                    f_iso_nir, f_vol_nir, f_geo_nir, tau
!---- end of namelist --------------------------------------------------------
...
subroutine read_soil_namelist()
  ...
  do 
     read (nml_unit, nml=soil_nml, iostat=io, end=10)
     if (check_nml_error (io, 'soil_nml')==0) exit ! from loop
  enddo
10  close (nml_unit)
  write (logunit, nml=soil_nml)
...
subroutine soil_init ( id_lon, id_lat, id_band )
 ...
 character(len=256) :: restart_file_name
  ...
  ! assign groundwater parameters, if requested
  ...
  ! set dry soil albedo values
     ...
     f_iso(:,:,BAND_VIS) = f_iso_vis
     f_vol(:,:,BAND_VIS) = f_vol_vis
     f_geo(:,:,BAND_VIS) = f_geo_vis
     f_iso(:,:,BAND_NIR) = f_iso_nir
     f_vol(:,:,BAND_NIR) = f_vol_nir
     f_geo(:,:,BAND_NIR) = f_geo_nir
...
subroutine soil_diag_init ( id_lon, id_lat, id_band )
  ...
  id_lwc   = register_tiled_diag_field ( trim(module_name), 'soil_liq',  'bulk density of liquid water', 'kg/m3')
  id_swc   = register_tiled_diag_field ( trim(module_name), 'soil_ice',  'bulk density of solid water', 'kg/m3')
  id_psi   = register_tiled_diag_field ( trim(module_name), 'soil_psi',  'soil-water matric head', 'm')
  id_temp  = register_tiled_diag_field ( trim(module_name), 'soil_T',    'temperature',            'degK')
  id_ie    = register_tiled_diag_field ( trim(module_name), 'soil_rie',  'inf exc runf',            'kg/(m2 s)')
  id_sn    = register_tiled_diag_field ( trim(module_name), 'soil_rsn',  'satn runf',            'kg/(m2 s)')
  id_bf    = register_tiled_diag_field ( trim(module_name), 'soil_rbf',  'baseflow',            'kg/(m2 s)')
  id_nu    = register_tiled_diag_field ( trim(module_name), 'soil_rnu',  'numerical runoff',    'kg/(m2 s)')
  id_hie   = register_tiled_diag_field ( trim(module_name), 'soil_hie',  'heat ie runf',            'W/m2')
  id_hsn   = register_tiled_diag_field ( trim(module_name), 'soil_hsn',  'heat sn runf',            'W/m2')
  id_hbf   = register_tiled_diag_field ( trim(module_name), 'soil_hbf',  'heat bf runf',            'W/m2')
  id_hnu   = register_tiled_diag_field ( trim(module_name), 'soil_hnu',  'heat nu runoff',          'W/m2')
  id_evap  = register_tiled_diag_field ( trim(module_name), 'soil_evap', 'soil evap',            'kg/(m2 s)')
  id_excess= register_tiled_diag_field ( trim(module_name), 'sfc_excess','sfc excess pushed down',    'kg/(m2 s)')

  id_uptk_n_iter  = register_tiled_diag_field ( trim(module_name), 'uptake_n_iter',   'number of iterations for soil uptake')
  id_uptk = register_tiled_diag_field ( trim(module_name), 'soil_uptk', 'uptake of water by roots', 'kg/(m2 s)')
  id_psi_bot = register_tiled_diag_field ( trim(module_name), 'soil_psi_n',  'psi at bottom of soil column', 'm')
  id_sat_frac = register_tiled_diag_field ( trim(module_name), 'soil_fsat',  'fraction of soil area saturated at surface', '-')
  id_stor_frac = register_tiled_diag_field ( trim(module_name), 'soil_fgw',  'groundwater storage frac above base elev', '-')
  id_sat_depth = register_tiled_diag_field ( trim(module_name), 'soil_wtdep',  'depth below sfc to saturated soil', 'm')

  ! ---- end of compatibility section

  id_heat_cap = register_tiled_diag_field ( trim(module_name), 'soil_heat_cap', 'heat capacity of dry soil','J/(m3 K)')
  id_thermal_cond =  register_tiled_diag_field ( trim(module_name), 'soil_tcon', 'soil thermal conductivity', 'W/(m K)')
  
  id_type = register_tiled_static_field ( trim(module_name), 'soil_type',   'soil type', '-')
  id_tau_gw = register_tiled_static_field ( trim(module_name), 'tau_gw',   'groundwater residence time', 's')
  id_slope_l = register_tiled_static_field ( trim(module_name), 'slope_l',   'hillslope length', 'm')
  id_slope_Z = register_tiled_static_field ( trim(module_name), 'soil_rlief',   'hillslope relief', 'm')
  id_zeta_bar = register_tiled_static_field ( trim(module_name), 'zeta_bar',   'hillslope zeta bar', '-')
  id_e_depth = register_tiled_static_field ( trim(module_name), 'soil_depth',   'soil e-folding depth', 'm')
  id_vwc_wilt = register_tiled_static_field ( trim(module_name), 'soil_wilt',   'wilting water content', '-')
  id_vwc_fc = register_tiled_static_field ( trim(module_name), 'soil_fc',   'field capacity', '-')
  id_vwc_sat = register_tiled_static_field ( trim(module_name), 'soil_sat',   'soil porosity', '-')
  id_K_sat = register_tiled_static_field ( trim(module_name), 'soil_Ksat',   'soil sat. hydraulic conductivity', 'kg /(m2 s)')
  id_w_fc = register_tiled_static_field ( trim(module_name), 'w_fc',   'soil field capacity') 
  id_refl_dry_dir = register_tiled_static_field ( trim(module_name), 'refl_dry_dir', 'reflectance of dry soil for direct light')
  id_refl_dry_dif = register_tiled_static_field ( trim(module_name), 'refl_dry_dif', 'reflectance of dry soil for diffuse light')
  id_refl_sat_dir = register_tiled_static_field ( trim(module_name), 'refl_sat_dir', 'reflectance of saturated soil for direct light')
  id_refl_sat_dif = register_tiled_static_field ( trim(module_name), 'refl_sat_dif', 'reflectance of saturated soil for diffuse light')
  id_f_iso_dry = register_tiled_static_field ( trim(module_name), 'f_iso_dry',  'isotropic brdf weight, dry soil')
  id_f_vol_dry = register_tiled_static_field ( trim(module_name), 'f_vol_dry', 'volumetric brdf weight, dry soil')
  id_f_geo_dry = register_tiled_static_field ( trim(module_name), 'f_geo_dry', 'geometric brdf weight, dry soil')
  id_f_iso_sat = register_tiled_static_field ( trim(module_name), 'f_iso_sat', 'isotropic brdf weight, saturated soil')
  id_f_vol_sat = register_tiled_static_field ( trim(module_name), 'f_vol_sat', 'volumetric brdf weight, saturated soil')
  id_f_geo_sat = register_tiled_static_field ( trim(module_name), 'f_geo_sat', 'geometric brdf weight, saturated soil')
...
! ----------------------------------------------------------------------------
  ...
  if(num_l > 1) then
     do l = 1, num_l-1
        dt_e = 2 / ( dz(l+1)/thermal_cond(l+1) + dz(l)/thermal_cond(l)   )
...
!   if (id_swc > 0) call send_tile_data(id_swc,  soil%prog%ws/dz(1:num_l), diag)
    call send_tile_data(id_lwc,  soil%prog%wl/dz(1:num_l), diag)
    call send_tile_data(id_swc,  soil%prog%ws/dz(1:num_l), diag)
