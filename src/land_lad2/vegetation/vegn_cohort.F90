...
! ---- Nitrogen model related parameters
  ...
  real    :: QNleaf = 0.
  real    :: QNsw = 0.
  real    :: QNwood = 0. ! N of heart wood
  real    :: QNroot = 0. ! N of fine roots
  real    :: QNseed = 0. ! fecundityN = 0.
  real    :: N_uptake = 0.
  real    :: N_up_yr  = 0.0
...
!! turned off by Weng 2014-01-09
!  if(firstlayer > 0)then
!     area = bl/spdata(species)%LMA
!  else
!     area = bl/spdata(species)%LMA * ((layer-1)*1.0+1.0)   
!  endif

!! modified by Weng 2014-01-09
!  if(layer > 1)then  ! turned off 01-23-14
...
!    cc%treeBM     = sp%alphaBM * cc%dbh ** sp%thetaBM
     ...
     cc%br_max = 0.8 * sp%LAImax/sp%SRA * cc%crownarea
