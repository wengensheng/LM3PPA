...
character(len=*), private, parameter :: &
   version = '$Id: vegn_radiation.F90,v 1.1.2.3 2012/06/19 18:34:55 pjp Exp $', &
...
! for skipping cohorts
real :: stored_refl_dif(4,6,NBANDS), & ! reflectance of canopy for diffuse light
        stored_tran_dif(4,6,NBANDS), & ! transmittance of canopy for diffuse light
        stored_refl_dir(4,6,NBANDS), & ! reflectance of canopy for direct light
        stored_sctr_dir(4,6,NBANDS), & ! part of direct light that is scattered downward
        stored_tran_dir(4,6,NBANDS), & ! part of direct light that passes through the canopy unmolested
        stored_refl_lw(4,6),         & ! reflectance of canopy for long-wave (thermal) radiation 
        stored_tran_lw(4,6),         & ! transmittance of canopy for long-wave (thermal) radiation
        stored_extinct(4,6) 
...
#undef CHECK_SIZE
  ...
  stored_refl_dif = -9999
  stored_tran_dif = -9999
  stored_refl_dir = -9999
  stored_sctr_dir = -9999
  stored_tran_dir = -9999
  stored_refl_lw  = -9999
  stored_tran_lw  = -9999
  stored_extinct  = -9999
  do i = 1,vegn%n_cohorts
!     if(i>1)then
!         preLAI = vegn%cohorts(i-1)%lai
!     else
!         preLAI = -5.0
!     endif
!     if(stored_extinct(vegn%cohorts(i)%layer,vegn%cohorts(i)%species) < -9998.0 .OR. &
!        ABS (vegn%cohorts(i)%lai - preLAI) > 0.1)then
...
!!        put to the arrays
!         stored_refl_dif(vegn%cohorts(i)%layer, vegn%cohorts(i)%species, :) = vegn_refl_dif(i,:)
!         stored_tran_dif(vegn%cohorts(i)%layer, vegn%cohorts(i)%species, :) = vegn_tran_dif(i,:)
!         stored_refl_dir(vegn%cohorts(i)%layer, vegn%cohorts(i)%species, :) = vegn_refl_dir(i,:)
!         stored_sctr_dir(vegn%cohorts(i)%layer, vegn%cohorts(i)%species, :) = vegn_sctr_dir(i,:)
!         stored_tran_dir(vegn%cohorts(i)%layer, vegn%cohorts(i)%species, :) = vegn_tran_dir(i,:)
!         stored_refl_lw (vegn%cohorts(i)%layer, vegn%cohorts(i)%species)    = vegn_refl_lw(i)
!         stored_tran_lw (vegn%cohorts(i)%layer, vegn%cohorts(i)%species)    = vegn_tran_lw(i)
!         stored_extinct (vegn%cohorts(i)%layer, vegn%cohorts(i)%species)    = vegn%cohorts(i)%extinct
!    else
!!        get them from the arrays
!         vegn_refl_dif(i,:) = stored_refl_dif(vegn%cohorts(i)%layer, vegn%cohorts(i)%species, :) 
!         vegn_tran_dif(i,:) = stored_tran_dif(vegn%cohorts(i)%layer, vegn%cohorts(i)%species, :)
!         vegn_refl_dir(i,:) = stored_refl_dir(vegn%cohorts(i)%layer, vegn%cohorts(i)%species, :) 
!         vegn_sctr_dir(i,:) = stored_sctr_dir(vegn%cohorts(i)%layer, vegn%cohorts(i)%species, :) 
!         vegn_tran_dir(i,:) = stored_tran_dir(vegn%cohorts(i)%layer, vegn%cohorts(i)%species, :) 
!         vegn_refl_lw(i)    = stored_refl_lw (vegn%cohorts(i)%layer, vegn%cohorts(i)%species)    
!         vegn_tran_lw(i)    = stored_tran_lw (vegn%cohorts(i)%layer, vegn%cohorts(i)%species)    
!         vegn%cohorts(i)%extinct = stored_extinct (vegn%cohorts(i)%layer, vegn%cohorts(i)%species)
!    endif
